...
func opcodeHash256(op *ParsedOpcode, t *thread) error {
	...
	t.dstack.PushByteArray(primitives.Sha256d(buf))
	return nil
}
...
func opcodeCheckSig(op *ParsedOpcode, t *thread) error {
	...
	if !t.hasFlag(scriptflag.EnableSighashForkID) || !shf.Has(sighash.ForkID) {
		suscript = suscript.removeOpcodeByData(fullSigBytes)
		suscript = suscript.removeOpcode(script.OpCODESEPARATOR)
	}

	up, err := t.scriptParser.Unparse(suscript)
	if err != nil {
		return err
	}
	...
	pubKey, err := primitives.ParsePubKey(pkBytes)
	if err != nil {
		t.dstack.PushBool(false)
	...
	if t.hasAny(scriptflag.VerifyStrictEncoding, scriptflag.VerifyDERSignatures) {
		signature, err = primitives.ParseDERSignature(sigBytes)
	} else {
		signature, err = primitives.ParseSignature(sigBytes)
	}
	if err != nil {
		t.dstack.PushBool(false)
		return nil //nolint:nilerr // only need a false push in this case
...
type parsedSigInfo struct {
	signature       []byte
	parsedSignature *primitives.Signature
	parsed          bool
}
...
func opcodeCheckMultiSig(op *ParsedOpcode, t *thread) error {
	...
	for numSignatures > 0 {
		...
		var parsedSig *primitives.Signature
		if !sigInfo.parsed {
			if err := t.checkHashTypeEncoding(shf); err != nil {
				return err
			}
			...
			var err error
			if t.hasAny(scriptflag.VerifyStrictEncoding, scriptflag.VerifyDERSignatures) {
				parsedSig, err = primitives.ParseDERSignature(signature)
			} else {
				parsedSig, err = primitives.ParseSignature(signature)
			}
			sigInfo.parsed = true
			if err != nil {
				continue
		...
		// Parse the pubkey.
		parsedPubKey, err := primitives.ParsePubKey(pubKey)
		if err != nil {
			continue
		}
