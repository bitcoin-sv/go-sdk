...
func (p *DefaultOpcodeParser) Parse(s *script.Script) (ParsedScript, error) {
	prs := *s
	parsedOps := make([]ParsedOpcode, 0, len(prs))
	conditionalBlock := 0
	...
	for i := 0; i < len(prs); {
		...
		case script.OpRETURN:
			...
			if conditionalBlock == 0 {
				...
				// we add any remaining data as an unformatted blob so that suscript can be reconstructed
				totalLen := len(prs)
				if (i + 2) > totalLen {
		...
		case parsedOp.op.length == 1:
			i++
		case parsedOp.op.length > 1:
			if len(prs[i:]) < parsedOp.op.length {
				return nil, errs.NewError(errs.ErrMalformedPush, "opcode %s required %d bytes, script has %d remaining",
					parsedOp.Name(), parsedOp.op.length, len(prs[i:]))
			}
			parsedOp.Data = prs[i+1 : i+parsedOp.op.length]
			i += parsedOp.op.length
		case parsedOp.op.length < 0:
			var l uint
			offset := i + 1
			if len(prs[offset:]) < -parsedOp.op.length {
				return nil, errs.NewError(errs.ErrMalformedPush, "opcode %s required %d bytes, script has %d remaining",
					parsedOp.Name(), parsedOp.op.length, len(prs[offset:]))
			}
			...
			case -1:
				l = uint(prs[offset])
			case -2:
				l = ((uint(prs[offset+1]) << 8) |
					uint(prs[offset]))
			case -4:
				l = ((uint(prs[offset+3]) << 24) |
					(uint(prs[offset+2]) << 16) |
					(uint(prs[offset+1]) << 8) |
					uint(prs[offset]))
			default:
			...
			if int(l) > len(prs[offset:]) || int(l) < 0 {
				return nil, errs.NewError(errs.ErrMalformedPush, "opcode %s pushes %d bytes, script has %d remaining",
					parsedOp.Name(), l, len(prs[offset:]))
			}

			parsedOp.Data = prs[offset : offset+int(l)]
...
func (p *DefaultOpcodeParser) Unparse(pscr ParsedScript) (*script.Script, error) {
	scr := make(script.Script, 0, len(pscr))
