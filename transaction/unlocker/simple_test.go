...
func TestLocalUnlocker_ValidSignature(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			uscript, err := unlocker.UnlockingScript(context.Background(), tx, transaction.UnlockerParams{})
			assert.NoError(t, err)
			...
			publicKey, err := primitives.ParsePubKey(publicKeyBytes)
			assert.NoError(t, err)

			sig, err := primitives.ParseDERSignature(sigBytes)
			assert.NoError(t, err)
...
func (m *mockUnlockerGetter) Unlocker(ctx context.Context, lockingScript *script.Script) (transaction.Unlocker, error) {
...
func (m *mockUnlocker) UnlockingScript(ctx context.Context, tx *transaction.Tx, params transaction.UnlockerParams) (*script.Script, error) {
...
// 	// fmt.Println(txid)
//
// 	var sigs = make([]*primitives.Signature, 2)
// 	var sigHashTypes = make([]uint32, 2)
// 	var publicKeys = make([]*primitives.PublicKey, 3)
//
...
//
// 	publicKeys[0], err = primitives.ParsePubKey(pk0, primitives.S256())
// 	if err != nil {
// 		t.Error(err)
// 		return
// 	}
// 	publicKeys[1], err = primitives.ParsePubKey(pk1, primitives.S256())
// 	if err != nil {
// 		t.Error(err)
// 		return
// 	}
// 	publicKeys[2], err = primitives.ParsePubKey(pk2, primitives.S256())
// 	if err != nil {
// 		t.Error(err)
// 		return
// 	}
//
// 	sigs[0], err = primitives.ParseDERSignature(sig0Bytes, primitives.S256())
// 	if err != nil {
// 		t.Error(err)
// 		return
// 	}
// 	sigs[1], err = primitives.ParseDERSignature(sig1Bytes, primitives.S256())
// 	if err != nil {
// 		t.Error(err)
// 		return
// 	}
