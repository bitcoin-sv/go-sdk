...
// their `PreviousTxScript` to the `transaction.UnlockerGetter` provided to the `transaction.FillAllInputs` call.
...
func main() {
	...
	if err = baseTx.Change(changeScript, transaction.NewFeeQuote()); err != nil {
		panic(err)
	}
	...
	if err := tx.Change(myAccount.createDestination(), transaction.NewFeeQuote()); err != nil {
		panic(err)
	}
